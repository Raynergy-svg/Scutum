...
# Installs + (re)loads the Sentinel LaunchAgent for the current user.
# Uses the modern launchctl bootstrap/bootout flow.

root_dir=$(cd "$(dirname "$0")/.." && pwd)
plist_src="$root_dir/launchd/com.irondome.sentinel.plist"
plist_dst="$HOME/Library/LaunchAgents/com.irondome.sentinel.plist"
uidn="$(id -u)"
...
/usr/bin/plutil -replace ProgramArguments.3 -string "$root_dir/scripts/irondome-sentinel-launch.zsh" "$plist_dst" >/dev/null
