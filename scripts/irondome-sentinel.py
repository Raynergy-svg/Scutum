...
class Sentinel:
    ...
    def _split_message(self, text: str) -> list[str]:
        ...
        for line in text.splitlines() or [text]:
            add = (line + "\n")
            if cur_len + len(add) > self.max_message_chars and cur:
                chunks.append("".join(cur).rstrip("\n"))
                cur = []
                cur_len = 0
            if len(add) > self.max_message_chars:
                # Hard split long lines.
                s = add
                while s:
                    chunks.append(s[: self.max_message_chars].rstrip("\n"))
                    s = s[self.max_message_chars :]
                continue
            cur.append(add)
            cur_len += len(add)
        ...
        # Add numbering only if it actually split.
        if len(chunks) > 1:
            numbered = []
            for i, c in enumerate(chunks, start=1):
                prefix = f"({i}/{len(chunks)}) "
                if len(prefix) + len(c) <= self.max_message_chars:
                    numbered.append(prefix + c)
                else:
                    numbered.append(prefix + c[: self.max_message_chars - len(prefix)])
            return numbered

        return chunks

    def _osascript_send(self, to_handle: str, message: str) -> None:
        # Mirrors scripts/irondome-imessage.zsh behavior.
        applescript = r'''
on run argv
  ...
  tell application "Messages"
    ...
    on error errMsg number errNum
      ...
      on error errMsg2 number errNum2
        error "iMessage failed and SMS unavailable. iMessage error: " & errMsg & " | SMS error: " & errMsg2 number errNum
      end try
    end try
  end tell
end run
'''
        try:
            subprocess.run(
                ["/usr/bin/osascript", "-", to_handle, message],
                input=applescript,
                text=True,
                check=True,
                capture_output=True,
            ...
            self._log("error", "osascript send failed", rc=e.returncode, stderr=(e.stderr or "")[:2000])

    # --------- Messages (receive/poll) ---------

    def poll_messages(self, buddy_handle: str, limit: int = 20) -> list[IncomingMessage]:
        """Return recent inbound messages from buddy_handle.

        Preferred backend is the local Messages database (`~/Library/Messages/chat.db`) via sqlite,
        which is more reliable than AppleScript polling under launchd.
        Set `SENTINEL_POLL_BACKEND=applescript` to force AppleScript polling.

        Notes:
        - We filter later by sender_handle.
        - Messages AppleScript can be flaky; errors are logged and treated as no new messages.
        """
